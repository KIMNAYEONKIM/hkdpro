...
# HKD parameters
parser.add_argument('--hkd_kd_weight', default=2.0, type=float)
parser.add_argument('--hkd_mse_weight', default=10.0, type=float)
parser.add_argument('--hkd_wcal_dim', default=32, type=int)
parser.add_argument('--hkd_layer_selection_method', default="positional_tolerance", type=str)
parser.add_argument('--hkd_aggregation_method', default="mean", type=str)
parser.add_argument('--hkd_position_tolerance', default=0.3, type=float)
parser.add_argument('--hkd_student_classes', default="BasicBlock,Sequential", type=str)
parser.add_argument('--hkd_projection_dim', default=128, type=int)
parser.add_argument('--hkd_num_t_layers', default=4, type=int)
parser.add_argument('--hkd_num_s_layers', default=4, type=int)
...
def main():
    ...
    if "hkd" in args.distiller:
        distiller = Distiller(model, teacher=teacher, criterion=train_loss_fn, args=args, num_data=len(dataset_train), input_size=[1]+list(data_config["input_size"]), num_classes=args.num_classes)
    else:
...
def train_one_epoch(
  ...
  File "/home/nayeonkim/Desktop/hkdpro/hkd-main/hkd_train.py", line 966, in <module>
    main()
  File "/home/nayeonkim/Desktop/hkdpro/hkd-main/hkd_train.py", line 719, in main
    train_metrics = train_one_epoch(
  File "/home/nayeonkim/Desktop/hkdpro/hkd-main/hkd_train.py", line 818, in train_one_epoch
 ...
 """
            ...
            for k in losses_dict:
                if isinstance(losses_dict[k],torch.Tensor):
                    loss_value = losses_dict[k].item()

                else:
                    loss_value = losses_dict[k]
                losses_m_dict[k].update(loss_value,input.size(0))
